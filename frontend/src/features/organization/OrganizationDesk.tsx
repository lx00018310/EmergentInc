...
function TrialArena(props: { trials: TrialDto[]; recruitments: any[]; members: QianjiListItemDto[]; selectedTrialId: string; setSelectedTrialId: (id: string) => void; run: (fn: () => Promise<unknown>) => Promise<void> }) {
  ...
  return <section className="org-panel">
    ...
    <form className="org-form" onSubmit={event => { event.preventDefault(); if (!props.recruitments.some(item => item.recruitmentId === recruitmentId)) return; void props.run(async () => { const trial = await api.createTrial({ recruitmentId, challengeText: challenge, acceptanceCriteria: criteria, totalBudgetTokens: totalBudget, candidateBudgetTokens: candidateBudget, roundsPerCandidate: trialRounds, modelName: 'gpt-4o-mini', allowedTools: ['save_artifact', 'read_artifact', 'list_artifacts'] }); props.setSelectedTrialId(trial.trialId); }); }}>
      <h3>建立统一试炼规则</h3><div className="org-fields"><label>招贤榜<select required value={recruitmentId} onChange={e => setRecruitmentId(e.target.value)}><option value="" disabled>请选择招贤榜</option>{props.recruitments.map(item => <option key={item.recruitmentId} value={item.recruitmentId}>{item.roleLabel}</option>)}</select></label><label>每位候选预算<input type="number" min={1} value={candidateBudget} onChange={e => setCandidateBudget(Number(e.target.value))} /></label><label>试炼总预算<input type="number" min={1} value={totalBudget} onChange={e => setTotalBudget(Number(e.target.value))} /></label><label>每人轮次<input type="number" min={1} value={trialRounds} onChange={e => setTrialRounds(Number(e.target.value))} /></label></div><label>统一考题<textarea value={challenge} onChange={e => setChallenge(e.target.value)} required /></label><label>统一验收标准<textarea value={criteria} onChange={e => setCriteria(e.target.value)} required /></label><button className="btn btn-xs" disabled={!props.recruitments.some(item => item.recruitmentId === recruitmentId)}>创建试炼</button>
      {props.recruitments.length === 0 && <p className="org-muted">请先发布招贤榜，再创建关联试炼。</p>}
    </form>
