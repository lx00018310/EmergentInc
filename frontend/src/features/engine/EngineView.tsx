...
export const EngineView: React.FC<{ onBack: () => void; initialPixelId?: string | null; onSelectedPixelChange?: (id: string | null) => void }> = ({ onBack, initialPixelId, onSelectedPixelChange }) => {
  ...
  // 元胞选择状态
  const [selectedPixelId, setSelectedPixelId] = useState<string | null>(initialPixelId ?? null);
  ...
  const handleSelectPixel = useCallback((id: string) => {
    ...
    onSelectedPixelChange?.(id);
  }, [onSelectedPixelChange]);

  const handleHoverPixel = useCallback((_id: string | null) => {}, []);

  return (
    <>
      <RunStatus world={world} runStatus={runStatus} audit={audit} onOpenHelp={() => setIsHelpModalOpen(true)} onBack={onBack} />
      ...
      <main className="main-layout">
        ...
        <section className="map-hero">
          ...
          <PixelDetails
            ...
            onClose={() => { setSelectedPixelId(null); onSelectedPixelChange?.(null); }}
          />
        </section>
      </main>
