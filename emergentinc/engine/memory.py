class MemoryService:
    ...
    def update(self,pixel_id,round_num,event):
        old=self.s.pixel_memory(pixel_id)
        delta,audit=self.llm.update_memory(pixel_id,round_num,old,event)
        mem=old
        ...
        for x in delta.get("memory_add",[]):
            if x not in mem.setdefault("working_memory",[]):
                mem["working_memory"].append(x)
