...
def approve(s,rid,cid,profile_file,reason='approved'):
    ...
    s.save_capability(cap); private=s.private/'capabilities'; private.mkdir(parents=True,exist_ok=True); (private/f'{cid}.json').write_text(json.dumps(profile,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
...
def main():
    ...
    sub=ap.add_subparsers(dest='cmd',required=True); sub.add_parser('list-requests')
    ...
    args=ap.parse_args(); s=Storage(args.workspace); s.ensure_v5_defaults()
