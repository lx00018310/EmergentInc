...
class RuleValidator:
    def __init__(self,storage):
        self.s=storage; self.cfg=self.s.config()["resource"]
    ...
    def action_cost(self,action):
        key={
          ...
          "REQUEST_CAPABILITY":"request_capability_cost","USE_CAPABILITY":"use_capability_cost",
          "WAIT_EXTERNAL":"wait_external_cost"
        }.get(action)
    ...
    def validate(self,pixel_id,a,local_view):
        ...
        if act=="WORK":
            ...
            if not isinstance(a.get("work_output"),dict) or not a["work_output"]:
        ...
        if act=="REQUEST_CAPABILITY":
            ...
            for k in ("capability_type","purpose","minimum_requirements","requested_operations"):
                if k not in req: return False,f"capability_request missing {k}"
        ...
        if act=="USE_CAPABILITY":
            ...
            cid=use.get("capability_id")
            available={c["id"]:c for c in local_view.get("external",{}).get("capabilities",[])}
            if cid not in available: return False,"capability not visible/owned"
            if use.get("operation") not in available[cid].get("allowed_operations",[]):
                return False,"operation not permitted"

        if act=="WAIT_EXTERNAL" and not isinstance(a.get("wait_external"),dict):
            return False,"missing wait_external"
