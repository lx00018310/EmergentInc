class EvidenceService:
    ...
    def request_close(self, problem_id):
        p=self.s.problem(problem_id)
        ...
        if len(eligible)<min_items:
            return {"verdict":"INSUFFICIENT","matched_acceptance":[],"missing_acceptance":p["acceptance"],"reason":"evidence source gate not satisfied"}, None
        verdict,audit=self.llm.validate_evidence_semantics(p,eligible)
