...
class Storage:
    ...
    def ensure_v5_defaults(self):
        for d in ['external_requests','capabilities','external_events','external_transactions','owner_private/capabilities']:
            (self.base/d).mkdir(parents=True,exist_ok=True)
        w=self.world(); w.setdefault('external_accounting',{'CNY_in':0.0,'CNY_out':0.0,'USD_in':0.0,'USD_out':0.0}); w.setdefault('counters',{})
        for k in ['external_requests','capabilities_granted','external_events']: w['counters'].setdefault(k,0)
        self.save_world(w)
    ...
    def _ids(self,folder,prefix): return sorted(p.stem for p in (self.base/folder).glob(f'{prefix}*.json'))
    ...
    def save_external_request(self,o): write_json(self.base/f'external_requests/{o["id"]}.json',o)
    ...
    def save_capability(self,o): write_json(self.base/f'capabilities/{o["id"]}.json',o)
    ...
    def save_event(self,o): write_json(self.base/f'external_events/{o["id"]}.json',o)
    ...
    def save_transaction(self,o): write_json(self.base/f'external_transactions/{o["id"]}.json',o)
    def create_pixel(self,pid,state,genome,memory):
        ...
        d.mkdir(parents=True); write_json(d/'state.json',state); write_json(d/'genome.json',genome); write_json(d/'memory.json',memory)
        for name,title in [('state.md','Pixel State'),('genome.md','Genome'),('memory.md','Memory'),('history.md','History'),('inbox.md','Inbox'),('llm_log.md','LLM Log')]:
