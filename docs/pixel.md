...
* 繁殖或在失活坐标重建时会创建新的 `state.json`，记录父代、代次、出生轮次和该坐标的生命序号 `incarnation`。旧文件与交付物移入 `live/history/<坐标>/<effect_id>/`；SQLite 中的历史账目保留。
...
6. 能量耗尽的 Pixel 失活：不再处理消息、划能或繁殖；收到 Owner 或活跃邻居给的任意正整数 Token 后恢复活跃，实际模型调用仍需足够预算。活跃邻居也可在该零余额位置繁殖新生命；旧文件与交付物归档，账目保留审计。
